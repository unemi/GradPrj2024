...
* 色分け
	...
	* $(1-\alpha)|h_a - H_f| / H_{max} + \alpha (S_{max} - s_a)/S_{max}, ~~ (0\leq\alpha\leq1)$ これはよくない！
		<br/>$h_a, s_a$: 相手の色相と彩度。$H_f$ 好みの色相。
...
##モデル
* 	彩度と好みの一致度の式を考える。
*  色相の差 $\Delta H$、彩度 $S$、色相の差の最大値を $\Delta\hat{H}$、彩度の最大値を $\hat{S}$ とすると、好みとの差 $D$ は

	$D=\Delta H / \Delta\hat{H}+ (\hat{S} - S)/\hat{S}$ ???
